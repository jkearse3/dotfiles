...
4. Select the AC marker from `references/ac-markers.md` § AC States based on observed satisfaction
   and confidence.
5. Record concise evidence per `references/ac-markers.md` § AC Validation Evidence for every AC
   moved to `[~]` or `[x]`.
