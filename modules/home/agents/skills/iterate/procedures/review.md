...
`Status: review`, `Next: none`: verified, awaiting human acceptance or follow-up.
...
- User accepts or closes: set `Status: complete`, `Next: none`; then return to normal runbook
  routing.
- User requests clear in-bound changes: treat as same-iteration feedback; update ACs/issues under AC
  Stability only as needed, set `Status: active`, set `Next: implement`, and return to normal
  runbook routing. Do not edit repo files inline for the feedback.
