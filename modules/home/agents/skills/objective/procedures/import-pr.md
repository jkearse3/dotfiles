...
6. Create phase file(s) — one per independent group — per `references/pr-review-import.md` § PR
   Review Import Conventions. When creating multiple phases, increment `P`, `NN`, and `M` for each
   subsequent group (e.g., first group gets P=3 NN=05 M=1, second gets P=4 NN=06 M=2); filenames
   stay distinct via the differing `P`/`NN`/`M` prefixes, not via a slug.

7. Register each phase with a linked index entry in `00-main.md` per
   `references/pr-review-import.md` § PR Review Import Conventions — one per phase created.
...
- Preserve branch detection, `gh-pr-comments`, unresolved-comment filtering, the comment JSON field
  list, independence grouping, phase-file writes, index registration, and GitHub metadata handling.
