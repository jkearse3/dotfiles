...
if command -v git >/dev/null 2>&1 && [[ $(git -C "$workspace" rev-parse --is-inside-work-tree 2>/dev/null) == true ]]; then
	...
	if [[ ${#worktrees[@]} -gt 1 && $first_record_bare == false && -d ${worktrees[0]}/.git ]]; then
		printf '%s\n' "${worktrees[0]}"
		exit 0
	fi
	if [[ ${#worktrees[@]} -gt 1 ]]; then
		if $first_record_bare; then
			printf '%s\n' 'resolve-store: bare-backed Git repository has multiple worktrees; choose a canonical workspace' >&2
		else
			printf '%s\n' 'resolve-store: unsupported non-bare Git topology does not expose a canonical primary worktree with a .git directory; choose a canonical workspace' >&2
		fi
		exit 3
	fi

	printf '%s\n' 'resolve-store: Git did not report a non-bare worktree' >&2
	exit 1
fi

# Pure jj repositories use the named default workspace when one exists.
...
# Without a default workspace, only a single workspace is unambiguous.
workspaces=()
