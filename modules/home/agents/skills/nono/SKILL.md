...
- After changing `.nono/profile.json`, tell the user to restart the wrapped agent/sandbox.

## Pitfalls

- `filesystem.bypass_protection` and `filesystem.suppress_save_prompt` do not grant filesystem
  access.
- `commands.*` config applies at startup. It does not grant command execution permission later.
