...
function __jj_bookmark_land_needs_tip
    ...
    argparse --ignore-unknown 'd/destination=' forget dry-run -- $tokens[2..] 2>/dev/null
    and test (count $argv) -eq 0
end

complete -c jj-bookmark-land -f
complete -c jj-bookmark-land -s h -l help -d 'Show help'
complete -c jj-bookmark-land -s d -l destination -r -f -d 'Destination bookmark' -a '(__jj_bookmark_land_bookmarks)'
