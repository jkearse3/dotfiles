...
from . import ensure as ensure_command
...
def _run(command: Sequence[str], *, cwd: Path | None = None) -> bytes:
    ...
    return checked_bytes(command, error_type=WorktreeAddError, cwd=cwd)
...
def _primary_worktree(start: Path) -> Path:
    output = _run(
        ["git", "-C", os.fspath(start), "worktree", "list", "--porcelain", "-z"]
    )
        ...
        if key == b"worktree" and separator:
            ...
            try:
                ...
                raise WorktreeAddError(
                    f"primary Git worktree is unavailable: {error}"
                ) from error
...
def add(name: str, revision: str = "@", start: Path | None = None) -> Path:
    ...
    try:
        _ = ensure_command.ensure(destination)
    except ensure_command.EnsureError as error:
        recovery = f"jjx ensure {shlex.quote(os.fspath(destination))}"
        message = (
...
def create_parser(prog: str = "jjx worktree add") -> argparse.ArgumentParser:
