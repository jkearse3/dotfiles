...
@dataclass(frozen=True)
class Inspection:
    """A validated checkout and the mutation required to ensure its jj state."""
...
class Plan:
    ...
    def describe(self) -> str:
        ...
        if self.git_identity is not None:
            signing = (
                "enabled" if self.git_identity.signing_key is not None else "disabled"
            )
...
def _run(command: Sequence[str], *, cwd: Path | None = None) -> bytes:
    """Run COMMAND and return stdout with concise launch/failure diagnostics."""
    return checked_bytes(command, error_type=EnsureError, cwd=cwd)
...
def _reject_git_lfs(checkout: GitCheckout) -> None:
    ...
    if configured.returncode == 0:
        ...
        attribute_files.append(
            ...
            else checkout.root / configured_path
        )
    ...
    for path in attribute_files:
        try:
            ...
            raise EnsureError(
                f"could not inspect Git attributes at {path}: {error}"
            ) from error
        for line in lines:
            fields = line.split()
            if (
...
def _worktree_paths(root: Path) -> list[Path]:
    ...
    output = _run(
        ["git", "-C", os.fspath(root), "worktree", "list", "--porcelain", "-z"]
    )
    paths: list[Path] = []
...
def _validate(checkout: GitCheckout) -> None:
    ...
    _same_file(
        _jj_path(checkout.root, "root"),
        checkout.root,
        "jj root does not match Git checkout",
    )
    _same_file(
...
def _identity_file(checkout: GitCheckout) -> Path:
    """Return the jjx ensure ownership marker in a worktree's private Git dir."""
...
def _ensure_identity(checkout: GitCheckout) -> None:
    ...
    try:
        ...
        try:
            ...
            raise EnsureError(
                f"could not inspect jjx ensure attachment identity: {error}"
            ) from error
        if not stat.S_ISREG(mode) or stat.S_ISLNK(mode) or actual != expected:
            raise EnsureError(
                "Git worktree has an incompatible jjx ensure attachment identity"
            )
    except OSError as error:
        ...
        suffix = (
            f"; cleanup failed: {cleanup_error}" if cleanup_error is not None else ""
        )
        raise EnsureError(
            f"could not record jjx ensure attachment identity: {error}{suffix}"
        ) from error
...
def _has_identity(checkout: GitCheckout) -> bool:
    ...
    if (
        ...
        or actual != os.fsencode(checkout.git_dir)
    ):
        raise EnsureError(
            "Git worktree has an incompatible jjx ensure attachment identity"
        )
...
def _read_stale_target(checkout: GitCheckout) -> Path:
    ...
    try:
        ...
        raise EnsureError(
            f"cannot inspect incompatible jj Git target: {error}"
        ) from error
    if not stat.S_ISREG(mode) or stat.S_ISLNK(mode):
...
def _validate_relocation(checkout: GitCheckout, stale: Path) -> None:
    ...
    if _read_identity(checkout) != stale:
        raise EnsureError(
            "stale jj target does not match this workspace's recorded attachment identity"
        )
    ...
    if (
        stale.parent.parent.name != ".git"
        or checkout.git_dir.parent.parent != checkout.common_git_dir
    ):
...
def _write_target_atomic(path: Path, value: Path) -> None:
    ...
    try:
        ...
        raise EnsureError(
            f"could not atomically repair jj Git target: {error}"
        ) from error
    finally:
...
def _repair(checkout: GitCheckout) -> None:
    ...
    try:
        ...
        _same_file(
            refreshed.git_dir,
            checkout.git_dir,
            "Git worktree identity changed during repair",
        )
        ...
        try:
            ...
            raise EnsureError(
                f"repair validation failed: {error}; rollback failed: {rollback_error}"
            ) from error
        raise EnsureError(
            f"repair validation failed; restored stale target: {error}"
        ) from error
...
def _default_remote_bookmark(checkout: GitCheckout) -> str | None:
    ...
    """
    remotes = os.fsdecode(
        _run(["git", "-C", os.fspath(checkout.root), "remote"])
    ).split()
    heads = {
        remote: branch
...
def _detect_default_bookmark(checkout: GitCheckout) -> str | None:
    ...
    try:
        ...
        print(
            f"warning: could not determine default remote bookmark: {error}",
            file=sys.stderr,
        )
        return None
...
def inspect(start: Path | None = None) -> Inspection:
    """Classify the validated checkout state without modifying it."""
    ...
    try:
        ...
        action = (
            Action.INITIALIZE_PRIMARY if checkout.primary else Action.INITIALIZE_LINKED
        )
        return Inspection(checkout, action)
    except OSError as error:
    ...
    try:
        ...
        stale = _read_stale_target(checkout)
        _validate_relocation(checkout, stale)
        return Inspection(checkout, Action.REPAIR, stale)
    if not checkout.primary and not _has_identity(checkout):
        return Inspection(checkout, Action.ENROLL)
    return Inspection(checkout, Action.NONE)
...
def plan(start: Path | None = None) -> Plan:
    ...
    """
    inspection = inspect(start)
    checkout = inspection.checkout
    track = (
        _detect_default_bookmark(checkout)
        if inspection.action in {Action.INITIALIZE_PRIMARY, Action.INITIALIZE_LINKED}
        else None
    )
    return Plan(
        checkout,
...
def ensure(start: Path | None = None) -> Path:
    ...
    if inspection.action is Action.REPAIR:
        _repair(checkout)
        fresh_init = False
    elif inspection.action is Action.ENROLL:
        _ensure_identity(checkout)
        fresh_init = False
    elif inspection.action is Action.NONE:
        fresh_init = False
    else:
        command = ["jj", "git", "init"]
        if inspection.action is Action.INITIALIZE_PRIMARY:
        ...
        try:
            ...
            _same_file(
                refreshed.git_dir,
                checkout.git_dir,
                "Git worktree identity changed during initialization",
            )
            if inspection.action is Action.INITIALIZE_LINKED:
                _ensure_identity(checkout)
        except EnsureError as error:
            suffix = _remove_created_jj(jj_dir)
            raise EnsureError(
                f"could not initialize jj repository: {error}{suffix}"
            ) from error
        fresh_init = True
...
def create_parser(prog: str = "jjx ensure") -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "mechanism. Retest jjx ensure after jj upgrades. After moving a primary repository, "
            "run 'git worktree repair' first; jjx ensure can then repair the linked workspace's "
...
def main(
    ...
    *,
    ...
    namespace = create_parser(prog).parse_args(arguments)
    try:
