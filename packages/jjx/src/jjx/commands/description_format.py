...
def _run(
    ...
    *,
    ...
    return capture_bytes(
        command,
        error_type=DescriptionFormatError,
        cwd=cwd,
        stdin=stdin,
    )
...
def _resolve_revision(revision: str, *, cwd: Path | None = None) -> str:
    """Resolve REVISION to one stable commit ID before reading or writing it."""
    matches = os.fsdecode(
        _run_ok(
            [
                "jj",
...
def _read_description(commit_id: str, *, cwd: Path | None = None) -> str:
    """Return the description of the exact COMMIT_ID."""
    return os.fsdecode(
        _run_ok(
            [
                "jj",
...
def _validate(text: str, *, subject_width: int | None, body_width: int | None) -> None:
...
def reformat(
    ...
    *,
    ...
    _ = _run_ok(
        ["jj", "describe", "-r", commit_id, "--stdin"],
        stdin=formatted.encode(),
        cwd=cwd,
    )
...
def create_parser(prog: str = "jjx description format") -> argparse.ArgumentParser:
...
def main(
    ...
    *,
    prog: str = "jjx description format",
) -> int:
