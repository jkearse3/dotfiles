...
from jjx.commands import ensure as cli


def run(
    *arguments: str, check: bool = True, cwd: Path | None = None
) -> subprocess.CompletedProcess[bytes]:
    return subprocess.run(
        arguments, cwd=cwd, check=check, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
class RepositoryFixture(unittest.TestCase):
    ...
    def private_git(self, path: Path) -> Path:
        return Path(
            run("git", "-C", str(path), "rev-parse", "--absolute-git-dir")
            .stdout.decode()
            .strip()
        )
    ...
    def cloned_with_remote(self, name: str) -> tuple[Path, str]:
        ...
        head = (
            run("git", "-C", str(clone), "symbolic-ref", "refs/remotes/origin/HEAD")
            .stdout.decode()
            .strip()
        )
    ...
    def tracked(self, path: Path) -> str:
        return run(
            "jj", "-R", str(path), "bookmark", "list", "--tracked"
        ).stdout.decode()
...
class EnsureBehaviorTests(RepositoryFixture):
    ...
    def test_rejects_git_lfs_before_initialization(self) -> None:
        _ = (self.primary / ".gitattributes").write_text(
    ...
    def test_dry_run_reports_noop_and_enrollment_without_writing_marker(self) -> None:
        ...
        self.assertIn(
            "would create linked-worktree identity marker", self.dry_run(linked)
        )
    ...
    def test_initializes_primary_from_subdirectory_and_is_idempotent(self) -> None:
        ...
        self.assertTrue(
            os.path.samefile(
                run("jj", "-R", str(self.primary), "git", "root")
                .stdout.decode()
                .strip(),
                self.primary / ".git",
            )
    ...
    def test_initializes_branch_and_detached_linked_worktrees(self) -> None:
        for branch in (False, True):
            ...
            symbolic_before = run(
                "git", "-C", str(linked), "symbolic-ref", "-q", "HEAD", check=False
            ).stdout
            self.assertEqual(cli.ensure(linked), linked)
            self.assertTrue(
                os.path.samefile(
                    run("jj", "-R", str(linked), "git", "root").stdout.decode().strip(),
                    self.private_git(linked),
                )
            )
            self.assertEqual(
                run(
    ...
    def test_resolves_symlink_and_survives_native_linked_move(self) -> None:
        ...
        _ = run(
            "git", "-C", str(self.primary), "worktree", "move", str(linked), str(moved)
        )
        self.assertEqual(cli.ensure(moved), moved)
        self.assertTrue(
            os.path.samefile(
                cli._jj_path(moved, "git", "root"), self.private_git(moved)
            )
        )
        ...
        self.assertEqual(cli.ensure(moved), moved)
        self.assertNotEqual(
            (moved / ".jj" / "repo" / "store" / "git_target").read_bytes(), stale
        )

    def test_enrolls_compatible_legacy_attachment_without_changing_jj_state(
        self,
    ) -> None:
        linked = self.linked("legacy")
        private = self.private_git(linked)
        ...
        self.assertEqual(
            (private / "jj-ensure-target").read_bytes(), os.fsencode(private)
        )
    ...
    def test_preserves_dirty_files_and_linked_state(self) -> None:
        ...
        contents = (
            (linked / "tracked").read_bytes(),
            (linked / "untracked").read_bytes(),
        )
        _ = cli.ensure(linked)
        operation = self.operation(linked)
        git_dir = self.private_git(linked)
        git_state = run(
            "git", "-C", str(linked), "status", "--porcelain=v1", "-z"
        ).stdout
        _ = cli.ensure(linked)
        self.assertEqual(
            ((linked / "tracked").read_bytes(), (linked / "untracked").read_bytes()),
            contents,
        )
        self.assertEqual(
            run("git", "-C", str(linked), "status", "--porcelain=v1", "-z").stdout,
            git_state,
        )
        self.assertEqual(self.operation(linked), operation)
    ...
    def test_repairs_after_primary_move_without_reinitializing_jj(self) -> None:
        ...
        self.assertEqual(self.operation(linked), operation)
        description = run(
            "jj", "-R", str(linked), "log", "-r", "@", "--no-graph", "-T", "description"
        ).stdout
        self.assertEqual(description, b"retained description\n")
        self.assertTrue(
            os.path.samefile(
                cli._jj_path(linked, "git", "root"), self.private_git(linked)
            )
        )
    ...
    def test_reconfigures_existing_workspace_without_rewriting_author(self) -> None:
        ...
        operation = self.operation(linked)
        _ = run(
            "git", "-C", str(self.primary), "config", "user.email", "moved@example.com"
        )
        self.assertEqual(cli.ensure(linked), linked)
    ...
    def test_dry_run_reports_mirrored_identity_without_writing_config(self) -> None:
        ...
        self.assertIn(
            "would mirror jj commit identity test@example.com (signing disabled)",
            output.getvalue(),
        )
        self.assertFalse((self.primary / ".jj").exists())
    ...
    def test_dry_run_predicts_default_remote_bookmark_before_init(self) -> None:
        ...
        self.assertIn(
            f"would track default remote bookmark {branch}@origin", output.getvalue()
        )
    ...
    def test_dry_run_detection_failure_warns_and_predicts_no_tracking(self) -> None:
        ...
        with (
            patch.object(
                cli,
                "_default_remote_bookmark",
                side_effect=cli.EnsureError("broken remote"),
            ),
        ...
        self.assertNotIn("would track", output.getvalue())
        self.assertIn(
            "warning: could not determine default remote bookmark: broken remote",
            error.getvalue(),
        )
        self.assertFalse((self.primary / ".jj").exists())
    ...
    def test_default_remote_bookmark_prefers_origin_across_remotes(self) -> None:
        ...
        for name, branch in (("origin", "main"), ("upstream", "dev")):
            _ = run(
                "git", "-C", str(self.primary), "remote", "add", name, f"../{name}.git"
            )
            _ = run(
                "git",
                "-C",
    ...
    def test_default_remote_bookmark_none_on_multi_remote_tie_without_origin(
        self,
    ) -> None:
        checkout = cli.discover(self.primary)
    ...
    def test_tracking_command_failure_warns_without_failing_ensure(self) -> None:
        ...
        with (
            ...
            self.assertEqual(cli.ensure(self.primary), self.primary)
        self.assertIn(
            "warning: could not track default remote bookmark main@", error.getvalue()
        )
        self.assertTrue((self.primary / ".jj").is_dir())
...
class EnsureSafetyTests(RepositoryFixture):
    ...
    def test_rejects_malformed_target_and_mismatched_identity_without_mutation(
        self,
    ) -> None:
        linked = self.linked("linked")
        _ = cli.ensure(linked)
        ...
        for target, identity in (
            ...
            with self.subTest(target=target):
                ...
                with self.assertRaises(cli.EnsureError):
                    _ = cli.ensure(linked)
                self.assertEqual(
                    (target_file.read_bytes(), identity_file.read_bytes()), before
                )
...
class CliTests(unittest.TestCase):
    def test_cli_prints_only_canonical_root_and_concise_error(self) -> None:
        ...
        with (
            patch.object(
                cli, "plan", side_effect=AssertionError("live execution called plan")
            ),
        ...
        with (
            patch.object(cli, "ensure", side_effect=cli.EnsureError("invalid target")),
            patch("sys.stderr", error),
        ):
