...
from jjx.commands import worktree_add as cli
...
class AddTests(RepositoryFixture):
    def test_creates_detached_worktree_at_default_revision(self) -> None:
        ...
        self.assertEqual(
            run("git", "-C", str(created), "rev-parse", "HEAD").stdout.strip(), source
        )
        self.assertNotEqual(
            run(
        ...
        private = Path(
            run("git", "-C", str(created), "rev-parse", "--absolute-git-dir")
            .stdout.decode()
            .strip()
        )
        self.assertTrue(
            os.path.samefile(
                run("jj", "-R", str(created), "git", "root").stdout.decode().strip(),
                private,
            )
        )
    ...
    def test_rejects_invalid_names_and_ambiguous_revisions_before_creation(
        self,
    ) -> None:
    ...
    def test_preserves_worktree_when_jjx_ensure_fails(self) -> None:
        error = cli.ensure_command.EnsureError("simulated failure")
        with patch.object(cli.ensure_command, "ensure", side_effect=error):
            with self.assertRaisesRegex(
                cli.WorktreeAddError, "preserved; recover with: jjx ensure"
            ):
        ...
        self.assertIn(
            ...
            [
                Path(line.split()[0])
                for line in run("git", "-C", str(self.primary), "worktree", "list")
                .stdout.decode()
                .splitlines()
            ],
        )
...
class CliTests(unittest.TestCase):
    def test_cli_prints_only_path_and_reports_concise_error(self) -> None:
        ...
        error = io.StringIO()
        with (
            patch.object(cli, "add", return_value=Path("/created")) as add,
        ...
        error = io.StringIO()
        with (
            patch.object(cli, "add", side_effect=cli.WorktreeAddError("failed")),
            patch("sys.stdout", output),
            patch("sys.stderr", error),
        ):
