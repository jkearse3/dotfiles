...
from jjx.commands import description_format as cli
...
class RepositoryFixture(unittest.TestCase):
    ...
    def operation(self) -> bytes:
        ...
        return run(
            "jj", "op", "log", "--no-graph", "-n", "1", "-T", "id", cwd=self.repository
        ).stdout
...
class ReformatTests(RepositoryFixture):
    ...
    def test_rejects_a_multi_revision_revset(self) -> None:
        ...
        with self.assertRaisesRegex(cli.DescriptionFormatError, "exactly one revision"):
            _ = cli.reformat("@ | @-", cwd=self.repository)
...
class StableTargetTests(unittest.TestCase):
    def test_reformat_writes_the_commit_resolved_before_formatting(self) -> None:
        with (
            patch.object(cli, "_resolve_revision", return_value="a" * 40) as resolve,
            patch.object(cli, "_read_description", return_value="subject\n") as read,
            patch.object(cli, "_format", return_value="Subject\n"),
            patch.object(cli, "_validate"),
            patch.object(cli, "_run_ok", return_value=b"") as run_ok,
            patch("sys.stdout", io.StringIO()),
        ):
            self.assertEqual(0, cli.reformat("moving-revset"))

        resolve.assert_called_once_with("moving-revset", cwd=None)
        read.assert_called_once_with("a" * 40, cwd=None)
        run_ok.assert_called_once_with(
            ["jj", "describe", "-r", "a" * 40, "--stdin"],
            stdin=b"Subject\n",
            cwd=None,
        )
