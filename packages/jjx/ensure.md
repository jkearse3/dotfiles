...
`jjx ensure` makes an existing Git checkout usable with
...
feature, so `jjx ensure` refuses to treat path names as proof of identity.
...
Retest `jjx ensure` after jj upgrades.

### The identity marker

When a linked workspace is initialized (or a compatible legacy one is enrolled),
`jjx ensure` writes a marker file, `jj-ensure-target`, into the worktree's
...
2. Run `jjx ensure` in the linked worktree.

`jjx ensure` rewrites `git_target` only when every check agrees: the workspace
is a linked worktree, the stale target no longer exists on disk, it exactly
matches the recorded identity marker, both old and new paths end in the same
...
agreement wherever the checkout lives, `jjx ensure` reads Git's resolved
...
`jjx ensure` runs that command for you.
...
- `src/jjx/commands/ensure.py` — the implementation; its module docstring is the
  authoritative behavioral summary.
- `tests/jj_ensure/test_jj_ensure.py` — behavior and safety tests against real
  `git` and `jj` binaries in temporary repositories.
- `completions/jjx.fish`, `completions/_jjx` — fish and zsh completions.
...
Run the tests directly (`git`, `jj`, `fzf`, `fish`, and `zsh` must be on `PATH`;
`PYTHONPATH=src` imports the working tree):

```console
$ PYTHONPATH=src python3 -B -m unittest discover -s tests -t . -p 'test_*.py'
$ PYTHONPATH=src python3 -B -m unittest discover -s tests/jjx -t . -p 'completion_test.py'
```
